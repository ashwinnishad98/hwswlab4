...
void dataAggregation(float newDistance) {
  ...
  Serial.println(globalMaxDistance);
  Serial.print("Min Distance: ");
  Serial.println(globalMinDistance);
}
...
void loop() {
  ...
  if (connected) {
    ...
    // float simulatedDistance = random(0, 100); 
    // pRemoteCharacteristic->writeValue((uint8_t*)&simulatedDistance, sizeof(float));
  }else if(doScan){
